    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-10 text-center md:text-left">
          <h1 className="text-[32px] md:text-[40px] font-black uppercase tracking-tighter text-[#1E3A8A] leading-none">Votre <span className="text-[#2563EB]">Panier</span></h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 items-start">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-none border-b-2 border-[#1E3A8A]/10 pb-4">
                ...
                <div key={item.id} className="bg-white shadow-sm hover:shadow-md transition-all duration-300 p-6 sm:p-8 flex flex-col sm:flex-row gap-8 items-center border border-gray-50 group">
                  ...
                  <div className="flex items-center gap-6 flex-1 w-full">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[14px] font-black uppercase tracking-tight text-[#1E3A8A] group-hover:text-[#2563EB] transition-colors">{item.name}</h3>
                  ...
                  <div className="hidden lg:grid grid-cols-3 gap-8 items-center w-full max-w-[450px]">
                    <div className="text-center text-[15px] font-black text-[#1E3A8A] tracking-tight">{formatPrice(item.price)}</div>

                    <div className="flex justify-center">
                      <div className="flex items-center gap-2 bg-gray-50 rounded-full p-1 border border-gray-100 shadow-inner">
                        <button
                          onClick={() => updateQuantity(item.cartItemId || item.id, item.quantity - 1)}
                          className="w-8 h-8 flex items-center justify-center rounded-full bg-white shadow-sm hover:text-[#2563EB] transition-colors border border-gray-100"
                        >
                          <Minus className="h-3 w-3" />
                        </button>
                        <span className="w-8 text-center text-[12px] font-black text-[#1E3A8A]">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.cartItemId || item.id, item.quantity + 1)}
                          className="w-8 h-8 flex items-center justify-center rounded-full bg-white shadow-sm hover:text-[#2563EB] transition-colors border border-gray-100"
                        >
                    ...
                    <div className="text-right text-[15px] font-black text-[#2563EB] tracking-tight">{formatPrice(item.price * item.quantity)}</div>
                  </div>
                  ...
                  <div className="lg:hidden flex items-center justify-between w-full pt-4 border-t border-gray-50 mt-2">
                    ...
                    <span className="text-[16px] font-black text-[#2563EB]">{formatPrice(item.price * item.quantity)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-16 pt-16 border-t border-gray-100">
              <div className="flex items-center justify-between mb-10 px-2">
                <div>
                  <h2 className="text-[20px] font-black uppercase tracking-tighter text-[#1E3A8A]">Vous pourriez aussi aimer</h2>
          ...
          <aside className="lg:sticky lg:top-24">
            <div className="bg-white shadow-2xl border border-gray-100 p-8 space-y-8">
              <div className="space-y-6">
                <h2 className="text-[16px] font-black uppercase tracking-widest text-[#1E3A8A] border-b border-gray-100 pb-4">Résumé de Commande</h2>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-[10px] font-black text-[#1E3A8A]">{Math.min(Math.round((totalPrice / freeShippingThreshold) * 100), 100)}%</span>
                  </div>
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                    <div
                      className="h-full bg-[#2563EB] transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(37, 99, 235,0.3)]"
                      style={{ width: `${Math.min((totalPrice / freeShippingThreshold) * 100), 100}%` }}
                    />
                  </div>
                  {remainingForFreeShipping > 0 ? (
                    <p className="text-[10px] font-bold text-gray-500 italic">Plus que <span className="text-[#2563EB] font-black">{formatPrice(remainingForFreeShipping)}</span> pour profiter de la livraison offerte !</p>
                  ) : (
                ...
                <div className="space-y-4 pt-4">
                  <div className="flex justify-between items-center text-[12px] font-bold text-gray-500">
                    ...
                    <span className="font-black text-[#1E3A8A]">{formatPrice(totalPrice)}</span>
                  </div>
                  <div className="flex justify-between items-center text-[12px] font-bold text-gray-500">
                    <span className="uppercase tracking-widest">Livraison</span>
                    <span className="font-black text-[#1E3A8A]">{totalPrice >= freeShippingThreshold ? "OFFERT" : formatPrice(shippingCost)}</span>
                  </div>
                </div>

                <div className="pt-6 border-t border-gray-100">
                  <div className="flex justify-between items-end">
                    <span className="text-[11px] font-black uppercase tracking-widest text-[#1E3A8A]">Total TTC</span>
                    <span className="text-[32px] font-black text-[#2563EB] tracking-tighter leading-none">
              ...
              <div className="space-y-4 pt-4">
                <button
                  ...
                  className="w-full bg-[#111827] hover:bg-[#2563EB] text-white hover:text-[#1E3A8A] font-black uppercase text-[12px] tracking-[0.25em] py-5 shadow-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group"
                >
                ...
                <Link
                  to="/boutique"
                  className="w-full h-14 border border-gray-200 text-gray-400 hover:text-[#1E3A8A] hover:border-[#1E3A8A] font-black uppercase text-[10px] tracking-widest transition-all flex items-center justify-center gap-2"
                >
